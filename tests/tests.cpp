...
void testForeignKey()
{
    ...
    storage.select(columns(&Visit::mark, &Visit::visited_at, &Location::place),
        ...
        where(is_equal(&Visit::user, id) and greater_than(&Visit::visited_at, fromDate) and lesser_than(&Visit::visited_at, toDate) and lesser_than(&Location::distance, toDistance)),
